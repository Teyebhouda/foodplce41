             ...
             ->editColumn('status', function ($user) {
              $status="";
                  ...
                  if($user->order_status==4){
                      if($user->shipping_type=='2'){
                           $status=__('messages.order_pickup');
                      }else{
   ...
   public function assignorder(Request $request){
     ...
     if($getorder){
      ...
      date_default_timezone_set($gettimezone);
           ...
           $data=$this->send_notification_android($keys->android_key,$request->get("assign_id"),$msg,"delivery_boyid");
         
            $this->send_notification_IOS($keys->ios_key,$request->get("assign_id"),$msg,"delivery_boyid");
